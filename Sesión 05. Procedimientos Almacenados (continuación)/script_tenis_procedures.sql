...
create or replace procedure pReservarPistas(
        p_socio VARCHAR,
        p_fecha DATE,
        p_hora INTEGER) is
        
        no_hay_pistas exception;
        pragma exception_init(no_hay_pistas, -20001);
        nro_no_hay_pistas constant varchar(70) := 'No quedan pistas libres en esa fecha y hora.';
...
begin
    ...
    THEN
        CLOSE vPistasLibres;
        raise_application_error(-20001, nro_no_hay_pistas);
    END IF;
...
create or replace procedure pAnularReservas(
    p_socio varchar,
	p_fecha date,
	p_hora integer, 
	p_pista integer) is
    ...
    pragma exception_init(reserva_inexistente, -20000);
    nro_reserva_inexistente constant varchar(50) := 'Reserva inexistente.';
...
begin
    ...
    if sql%rowcount = 1 then
        ...
        rollback;
        raise_application_error(-20000, nro_reserva_inexistente);
    end if;
end;
/
